...
import {
  ...
  PayableStatusEnum,
} from '../../../shared/payables/schemas.js';
import { sendError } from '../../lib/errors.js';
import type { PayableService } from './payables.service.js';
...
export class PayableController {
  ...
  list = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const parsedStatus = statusParam ? PayableStatusEnum.safeParse(statusParam) : null;
  ...
  create = async (req: Request, res: Response): Promise<void> => {
    try {
      const parsed = CreatePayableBodySchema.safeParse(req.body);
      if (!parsed.success) {
        res.status(400).json({ error: firstZodMessage(parsed.error, 'Invalid payable') });
        return;
      }
  ...
  update = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const parsed = UpdatePayableBodySchema.safeParse(req.body);
      if (!parsed.success) {
        res.status(400).json({ error: firstZodMessage(parsed.error, 'Invalid payable') });
        return;
      }
